...
export default function EditPostForm({ path }: { path: string | null }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (title.trim().length < 1 || body.trim().length < 1) {
      alert("make sure to fill in the post title and body!");
      return;
    }
    const res = await fetch("/api/admin/blog", { method: path ? "PATCH" : "POST", body: JSON.stringify({ title, body, blurb }) });
    let resBody;
    try {
    ...
    <form className="flex flex-col mt-4 gap-2 items-center" onSubmit={handleSubmit}>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div className="flex flex-col gap-1 md:col-span-2">
          ...
          <textarea id="post-body" className="bg-slate-300 dark:bg-slate-800 rounded-lg p-1" value={body} onChange={(e) => setBody(e.target.value)} rows={6} />
        </div>
      </div>
