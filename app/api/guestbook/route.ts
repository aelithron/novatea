...
export async function POST(req: NextRequest) {
  ...
  try {
    await db.insert(guestbookTable).values({ name: body.name as string, email: body.email as string, body: body.body as string, url: (body.url ? body.url as string : null), visible: false, createdAt: new Date(), reply: "" });
  } catch (e) {
    console.warn(e);
