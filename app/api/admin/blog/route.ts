...
export async function PATCH(req: NextRequest): Promise<NextResponse> {
  ...
  const updatedData: { title?: string, blurb?: string, body?: string, published?: boolean, publishedAt?: string } = {};
  ...
  if (body.published) updatedData.published = (body.published === true ? true : false);
  if (body.publishedAt) updatedData.publishedAt = new Date(body.publishedAt as string).toISOString();
  if (JSON.stringify(updatedData) === "{}") return new NextResponse(null, { status: 204 });
  try {
