...
export async function PATCH(req: NextRequest): Promise<NextResponse> {
  ...
  if ((!body.visible || (body.visible !== true && body.visible !== false)) && (body.reply === undefined)) NextResponse.json({ error: "missing_action", message: "'visible' and 'reply' are both missing or invalid, please send a valid boolean visible attribute and/or valid reply string attribute!" }, { status: 400 });
  try {
  ...
  try {
    await db.update(guestbookTable).set({ visible: (body.visible !== undefined ? body.visible as boolean : undefined), reply: (body.reply !== undefined ? body.reply as string : undefined) }).where(eq(guestbookTable.id, Number.parseInt(body.id as string)));
